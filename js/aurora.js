...
Asset = (function(_super) {
  ...
  Asset.prototype.decodeToBuffer = function(callback) {
    ...
    length = 0;
    chunks = [];
// AT: once('end'...) handler moved here to make it applicable to this.stop() as well due to large MP3
var on_ended = function() {
      var buf, chunk, offset, _i, _len;
// AT: prevent "Array Buffer allocation failed" error in Wallpaper Engine
_len = chunks.length
var allocation_done
var length_full = length
while (!allocation_done && _len) {
  try {
...
length -= chunks[--_len].length
  }
}
console.log("buffer size adjusted:"+length+'/'+length_full)
//DEBUG_show(length+'/'+length_full,0,1)
      offset = 0;
...
// AT: stop decoding at certain length limit (20M, to be safe), to prevent random crash
if (length+chunk.length > 20000000) {
  console.log("decoding stopped:"+length)
//  DEBUG_show(length,0,1)
  this.stop()
  var that = this
  setTimeout(function () { on_ended.call(that) }, 100)
  return chunks
}
      length += chunk.length;
      return chunks.push(chunk);
    });
    this.once('end', on_ended);
    return this.start();
  };
...
// AT: 200 => 50
    this._timer = setInterval(this.updateTime, 50);
