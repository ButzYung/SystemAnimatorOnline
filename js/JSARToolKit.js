...
// AT: d_length
    for (var i=0,j=0,d_length=d.length; i<d_length; i+=4,++j) {
...
/*
// AT: WebGL processing
// in_reader.getData() (getImageData) skipped (because it's SLOW), assuming that it is NEVER used outside this function (which seems true so far)
var w2d = i_input._canvas._WebGL_2D
var gl
if (w2d._AR_threshold != this._threshold) {
  w2d._AR_threshold = this._threshold;
  w2d.init();
  gl = w2d.gl;
  gl.uniform1f(gl.getUniformLocation(w2d.program_2d, "_threshold"), w2d._AR_threshold);
}
else {
  gl = w2d.gl;
}
...
var _cb  = w2d._AR_canvas_buffer
var _ctx = _cb.getContext("2d")
_ctx.globalCompositeOperation = 'copy'
_ctx.drawImage(w2d.canvas, 0,0)
var _b8  = _ctx.getImageData(0,0,_cb.width,_cb.height).data.buffer
var _b32 = new Uint32Array(_b8)

//var w = w2d.canvas.width; var h = w2d.canvas.height; var _b8 = w2d._b8 = w2d._b8 || new Uint8Array(w*h*4); gl.readPixels(0,0,w,h, gl.RGBA, gl.UNSIGNED_BYTE, _b8); var _b32 = new Uint32Array(_b8);
...
out_buf.data = _b32;
        ...
        context.drawImage(_cb, 0,0)
        if (SL_webcam._WebGL_2D)
          SL_webcam._WebGL_2D.draw()
*/
//MMD_SA._debug_msg = [performance.now() - _t]
...
doThFilterImpl_BUFFERFORMAT_OBJECT_AS_BitmapData = {
  doThFilter : function(i_input, i_output, i_threshold)
  {
    ...
    var obd = out_buf.data;
// AT: d_length
    for (var i=0,d_length=d.length; i<d_length; i++) {
      var dc = d[i];
