...
if (typeof(JSIO) != 'boolean') {
	...
	var FileIO = {
		...
		read   : function(file, charset, length) {
			try {
				var data     = new String();
				...
				//SA mod
				if (!length) { length = siStream.available() };

//SA mod: read(-1) => readBytes
				data += siStream.readBytes(length);
				siStream.close();
				fiStream.close();
				if (charset) {
...
readText: function (file, charset) {
	try {
		if (!charset)
			charset = "UTF-8"
...
var charset_alt = 0x0020; //Components.interfaces.nsIConverterInputStream.DEFAULT_REPLACEMENT_CHARACTER
		cstream.init(fstream, charset, 0, charset_alt);
