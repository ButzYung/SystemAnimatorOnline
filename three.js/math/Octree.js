...
// AT: increase limit from 8 to 24
			if ( len > 24 && level < 16 ) {
...
// AT: ignore_backfaceCulling
	rayIntersect( ray, ignore_backfaceCulling ) {
...
const _rot = group.quaternion.clone();
const _scale = group.scale.clone();
...
if (!MMD_SA.THREEX.enabled) {
  if (obj.isMesh) {
    ...
    for (let f = 0, fl = geometry.faces.length; f < fl; f++) {
      ...
      const v1 = new Vector3().copy(vertices[face.a]);
      const v2 = new Vector3().copy(vertices[face.b]);
      const v3 = new Vector3().copy(vertices[face.c]);
