...
if (MMD_SA._hit_chest_) {
  ...
  this._cover_chest_=false
   ...
   ,motion: {path:'MMD.js/motion/cover_chest_v02b.vmd', match:{bone_group:["腕","指"], all_morphs:true, skin_jThree:/^(\u5DE6|\u53F3)(\u80A9|\u8155|\u3072\u3058|\u624B\u9996|\u624B\u6369|.\u6307.)/, morph_jThree:true}}
  ...
  MMD_SA_options.motion.push(
    { must_load:true, no_shuffle:true, path:'MMD.js/motion/hit/r01_普通に転ぶ.vmd' }
   ,{ must_load:true, no_shuffle:true, path:'MMD.js/motion/casual/OTL→立ち上がり.vmd' }
  );
  var _motion_para = {
    "r01_普通に転ぶ" : { adjust_center_view_disabled:true, get look_at_screen_ratio() { var f=THREE.MMD.getModels()[0].skin.time*30; return ((f>10)?0:(10-f)/10); }
     ,onended: function () {
MMD_SA._ignore_physics_reset=true; MMD_SA._no_fading=true;
      }
     ,adjustment_per_model: {
  ...
  "全ての親": { keys:[{ time:0/30, pos:{x:0, y:0, z:0} }, { time:19/30, pos:{x:0, y:1.5, z:-8}, rot:{x:-5, y:0, z:0} }] }
   ...
   ,"OTL→立ち上がり" : { adjust_center_view_disabled:true, get look_at_screen_ratio() { var f=THREE.MMD.getModels()[0].skin.time*30; return ((f<29)?0:((f>47)?1:(f-29)/18)); }
     ,onended: function () {
MMD_SA._hit_legs_=false; MMD_SA._no_fading=true;
MMD_SA_options.motion_shuffle_list_default = MMD_SA_options._motion_shuffle_list_default.slice();
MMD_SA_options.Dungeon._states.character_movement_disabled = false;
MMD_SA_options.Dungeon._states.object_click_disabled = false;
  ...
  Object.assign(MMD_SA_options.motion_para, _motion_para);
...
var bb_max_y = THREE.MMD.getModels()[0].mesh.geometry.boundingBox.max.y

if (intersected.y > bb_max_y*0.9)
  MMD_SA._hit_head_ = true
else if (intersected.y > bb_max_y*0.7)
...
else if (intersected.y < bb_max_y*0.3) {
  if (!pressed) {
    MMD_SA._hit_legs_ = true
    d._states.character_movement_disabled = true
...
MMD_SA_options.Dungeon_options.motion = {
   ...
   ,"NPC talk A": { path:System.Gadget.path + '/MMD.js/motion/talk/xs-talk8-west-negotiate.vmd'
     ,para: {
  model_index_list:[1], onended: function () { MMD_SA._no_fading=true; }
 ,adjustment_per_model: {
    _default_ : {
  skin_default: {
    "全ての親": { keys:[{ pos:{x:1.05, y:0, z:-8.72}, rot:{x:0, y:89, z:0} }] }
...
if (!MMD_SA_options.Dungeon._states.event_mode) {
 ...
 ,"左足ＩＫ": { keys_mod:[{ frame:0, pos:{x: 2.33+1, y:-0.4, z:1.3} }, { frame:47, pos:{x: 2.33+1, y:-0.4, z:1.3} }] }
   ...
   ,"■宮本武蔵霊基再臨01.pmx" : {
  skin_default: {
  "右足ＩＫ": { keys_mod:[{ frame:0, pos:{x:-2.33-2, y:-0.4, z:1.3} }, { frame:7,  pos:{x:-2.33-2, y:-0.4, z:1.3} }] }
 ,"左足ＩＫ": { keys_mod:[{ frame:0, pos:{x: 2.33+2, y:-0.4, z:1.3} }, { frame:47, pos:{x: 2.33+2, y:-0.4, z:1.3} }] }
  }
    }
  }
    }}
