...
var wallpaper_src;
async function onDrop_change_wallpaper(item) {
  var src = item.path
  if (item.isFileSystem && /([^\/\\]+)\.json$/i.test(src)) {
    onDrop_scene_JSON(item);
  }
  else if (item.isFileSystem && (/([^\/\\]+)\.(png|jpg|jpeg|bmp|webp)$/i.test(src) || (MMD_SA.Wallpaper3D.enabled && /([^\/\\]+)\.(mp4|mkv|webm)$/i.test(src)))) {
    wallpaper_src = src;
    ...
    if (wallpaper) {
      ...
      if (wallpaper.path) {
        if (await locate_file(zip, wallpaper)) {
          ...
          onDrop_change_wallpaper({ isFileSystem:true, path:wallpaper.path }).then(()=>{
            if (wallpaper.settings_3d) {
...
//  MMD_SA_options._XRA_onDrop_scene_JSON = onDrop_scene_JSON;
});
...
return true;
   ...
   ,{ key:6, is_closing_event:true, func:()=>{ wallpaper_dialog_enabled=false; }, branch_index:bg_branch+6 }
  ]
            },
            next_step: {}
          },
...
'F.    ┗ ' + System._browser.translation.get('XR_Animator.UI.UI_options.scene.background.3D_wallpaper.scale_z.depth_blur')  + ': ' + ((MMD_SA.Wallpaper3D.enabled) ? MMD_SA.Wallpaper3D.options.depth_blur  : 'N/A') + ((LR_option_active == 'depth_blur') ?'⬅️➡️':'  　　'),
...
'J. ┗ ' + System._browser.translation.get('XR_Animator.UI.UI_options.scene.background.3D_wallpaper.export_scene_to_file')
    ].join('\n');
  },
  index: 1,
  bubble_index: 3,
...
MMD_SA_options.Dungeon.utils.tooltip(
  ...
  System._browser.translation.get('XR_Animator.UI.motion_capture.mocap_options.face_tracking_options.auto_blink.tooltip')
    ...
    { key:5, event_id:{ func:()=>{ System._browser.camera.facemesh.auto_look_at_camera = !System._browser.camera.facemesh.auto_look_at_camera; }, goto_branch:facemesh_options_branch },
      onmouseover: function (e) {
MMD_SA_options.Dungeon.utils.tooltip(
  e.clientX, e.clientY,
  System._browser.translation.get('XR_Animator.UI.motion_capture.mocap_options.face_tracking_options.auto_look_at_camera.tooltip').replace(/\<hotkey\>/, System._browser.hotkeys.config_by_id['auto_look_at_camera']?.accelerator[0]||'')
    ...
    { key:6, event_id:{ func:()=>{}, goto_branch:facemesh_options_branch },
      onmouseover: function (e) {
MMD_SA_options.Dungeon.utils.tooltip(
  e.clientX, e.clientY,
...
return [
  ...
  '3. ' + System._browser.translation.get('XR_Animator.UI.motion_capture.mocap_options.face_tracking_options.blink_clarity') + ': ' + blink_clarity,
  ...
  '5. ' + System._browser.translation.get('XR_Animator.UI.motion_capture.mocap_options.face_tracking_options.auto_look_at_camera') + ' (' + (System._browser.hotkeys.config_by_id['auto_look_at_camera']?.accelerator[0]||'') + '): ' + ((!System._browser.camera.facemesh.auto_look_at_camera)?'OFF':'ON'),
  '6. ' + System._browser.translation.get('XR_Animator.UI.motion_capture.mocap_options.face_tracking_options.eye_bone_rotation') + ': ' + (System._browser.camera.facemesh.eye_bone_rotation_percent + '%') + '➕➖',
  '7. ' + System._browser.translation.get('XR_Animator.UI.motion_capture.mocap_options.face_tracking_options.mouth_tracking_sensitivity') + ': ' + mouth_tracking_sensitivity,
  '8. ' + System._browser.translation.get('XR_Animator.UI.motion_capture.mocap_options.face_tracking_options.lean_tracking') + ': ' + (lean_tracking || 'OFF'),
  '9. ' + System._browser.translation.get('XR_Animator.UI.motion_capture.mocap_options.face_tracking_options.emotion_tracking_options') + ' ▶️',
  ...((System._browser.camera.facemesh.enabled && System._browser.camera.video) ? ['0. ' + System._browser.translation.get('XR_Animator.UI.motion_capture.mocap_options.face_tracking_options.calibration_options') + ' ▶️'] : []),
  'X. ' + System._browser.translation.get('Misc.done'),
].join('\n');
  }
...
MMD_SA.Wallpaper3D._access_general_options_only_ = true;
const wallpaper_3d_config = {};
for (const p of ['enabled', 'scale_xy_percent', 'scale_z_percent', 'depth_shift_percent', 'depth_contrast_percent', 'depth_blur', 'pos_x_offset_percent', 'pos_y_offset_percent', 'pos_z_offset_percent']) {
  wallpaper_3d_config[p] = MMD_SA.Wallpaper3D.options[p];
}
config.wallpaper_3d = wallpaper_3d_config;
MMD_SA.Wallpaper3D._access_general_options_only_ = false;
...
//        if (MMD_SA_options.Dungeon.started && (is_global_now != hotkeys.is_global)) System._browser.hotkeys.register_global(hotkeys.is_global);
      ...
      case 'wallpaper_3d':
        MMD_SA.Wallpaper3D._access_general_options_only_ = true;
        for (const _p of ['enabled', 'scale_xy_percent', 'scale_z_percent', 'depth_shift_percent', 'depth_contrast_percent', 'depth_blur', 'pos_x_offset_percent', 'pos_y_offset_percent', 'pos_z_offset_percent']) {
          MMD_SA.Wallpaper3D.options[_p] = config[p][_p];
        }
        MMD_SA.Wallpaper3D._access_general_options_only_ = false;
        break;
