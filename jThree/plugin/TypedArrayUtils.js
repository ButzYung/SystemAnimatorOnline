...
// AT: stuff
THREE.TypedArrayUtils._distanceFunction = function ( para, mesh ) {
  var dx, dy, dz, dd, v

  dx = para[0] - mesh[0]
  dy = Math.min(para[1] - mesh[1], 0)
  dz = para[2] - mesh[2]
  dd = mesh[3]

// NOTE: distance should always be NON-NEGATIVE for kd-tree search to function properly.
// (x^2 + y^2 + z^2) < (r + maxDistance)^2
// (x^2 + y^2 + z^2) < r^2 + 2*r*maxDistance + maxDistance^2
  return Math.max(Math.sqrt(dx*dx + dy*dy + dz*dz) - dd + 999, 0);
/*
  v = (dx*dx + dy*dy + dz*dz) - (dd*dd + 2*dd*THREE.TypedArrayUtils._maxDistance_)
  if (v < 1) {
    v = v/999999 + 0.5
  }
  return v
...
THREE.TypedArrayUtils._result_func = function (result, bestNodes, i) {
  result.push(bestNodes.content[ i ][ 0 ].obj[4])
};
...
// AT: result_func, _maxDistance_
var _result_func = function(_result,_bestNodes,_i){_result.push( [ _bestNodes.content[ _i ][ 0 ], _bestNodes.content[ _i ][ 1 ] ] );};
	this.nearest = function ( point, maxNodes, maxDistance ,result_func) {// ){//
if (!result_func) result_func=_result_func
maxDistance += 999
//THREE.TypedArrayUtils._maxDistance_ = Math.sqrt(maxDistance||0)
...
// AT: result_func
result_func(result,bestNodes,i)
