...
THREE.SAOShader = {
	...
	uniforms: {
		...
		"scale":        { type: "f", value: 1.0 +200},
		"intensity":    { type: "f", value: 0.1 *1},
		"bias":         { type: "f", value: 0.5 +0.25*1},

		"minResolution": { type: "f", value: 0.0 },
		"kernelRadius": { type: "f", value: 100.0 -50},
...
if (refresh_all_uniforms) {
  ...
  e.uniforms[ 'scale' ].value = 1.0 +200 * camera.far/2000
  e.uniforms[ 'cameraInverseProjectionMatrix' ].value.getInverse( camera.projectionMatrix );
  e.uniforms[ 'cameraProjectionMatrix' ].value = camera.projectionMatrix;
}
